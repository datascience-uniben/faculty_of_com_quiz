...
def draw_random_question():
    """Picks one random question and removes it entirely from the pool."""
    if st.session_state.question_pool:
        # Since the pool is pre-shuffled, popping off the end gives a safe random selection
...
if st.session_state.round_active and st.session_state.round_team:
    ...
    if not st.session_state.has_drawn_question:
        st.info("💡 Ready for your turn? Click the button below to draw a random question from the remaining category pool.")
        if st.button("🎲 Draw Next Question", type="primary"):
            draw_random_question()
            st.rerun()
            
    else:
        # Question has been pulled, render it to the console
        ...
        if q:
            ...
            choice = st.radio("Choose Your Team's Definitive Answer:", options, index=None, key="current_options_radio")
