...
def render_live_monitoring_view():
    ...
    for i, stage in enumerate(stages_meta):
        with cols[i]:
            ...
            if len(ranked_teams) >= cutoff:
                ...
                st.metric(label=stage["title"], value=f"Top {cutoff} Qualified", delta=f"Cutoff: Team {borderline_team}")
            else:
    ...
    with col1:
        ...
        if not df_scores.empty and df_scores.iloc[0]["Total Score"] > 0:
            st.success(f"🌟 **Leader:** Team {df_scores.iloc[0]['Team']} ({df_scores.iloc[0]['Total Score']} pts)")
    ...
    with col2:
        st.subheader("📊 Round Tally Matrix (Nigeria Current Affairs)")
        ...
        for team in ALL_TEAMS:
            ...
            def extract_round_score(round_str):
                if team_logs.empty: return "⏳ Pending"
                ...
                if stage_col and score_col:
                    ...
                    if not match_row.empty: return f"{match_row.iloc[0][score_col]} pts"
                return "⏳ Pending"
